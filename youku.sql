...
CREATE TABLE `advert` (
  ...
  `channel_id` int DEFAULT '0' COMMENT '所属频道 0=首页',
...
CREATE TABLE `barrage` (
  ...
  `add_time` int DEFAULT '0' COMMENT '评论时间',
  `user_id` int DEFAULT '0' COMMENT '评论用户',
  `status` tinyint DEFAULT '0' COMMENT '状态0=未审核 1=审核通过',
  `current_time` int DEFAULT '1' COMMENT '视频当前播放时间',
  `episodes_id` int DEFAULT '0' COMMENT '评论视频',
  `video_id` int DEFAULT '0' COMMENT '所属视频',
  PRIMARY KEY (`id`)
...
CREATE TABLE `channel_region` (
  ...
  `add_time` int DEFAULT '0' COMMENT '添加时间',
  `channel_id` int DEFAULT NULL COMMENT '所属频道',
...
CREATE TABLE `comment` (
  ...
  `user_id` int DEFAULT '0' COMMENT '评论用户',
  `status` tinyint DEFAULT '0' COMMENT '状态0=未审核 1=审核通过',
  `stamp` tinyint DEFAULT '0' COMMENT '盖章1=热评2=公告',
  `praise_count` int DEFAULT '0' COMMENT '点赞数',
  `episodes_id` int DEFAULT NULL COMMENT '评论视频',
  `video_id` int DEFAULT '0' COMMENT '所属视频',
  PRIMARY KEY (`id`)
...
CREATE TABLE `message_user` (
  `id` int NOT NULL AUTO_INCREMENT,
  `message_id` int DEFAULT '0' COMMENT '所属消息',
  `user_id` int DEFAULT '0' COMMENT '所属用户',
  `add_time` int DEFAULT '0' COMMENT '添加时间',
  `status` tinyint DEFAULT '0' COMMENT '状态 1=展示',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=47 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
CREATE TABLE `user` (
  ...
  `add_time` int DEFAULT '0' COMMENT '注册时间',
...
CREATE TABLE `video` (
  ...
  `status` tinyint DEFAULT '0' COMMENT '状态,-1=下架,0=审核中,1=正常',
  `add_time` int DEFAULT '0' COMMENT '添加时间',
  `img` varchar(255) DEFAULT NULL COMMENT '横版缩略图',
  `img1` varchar(255) DEFAULT NULL COMMENT '竖版缩略图',
  `channel_id` int DEFAULT '0' COMMENT '所属频道',
  `type_id` int DEFAULT '0' COMMENT '频道类型ID',
  `region_id` int DEFAULT '0' COMMENT '地区ID',
  `user_id` int DEFAULT '0' COMMENT '所属用户',
  `episodes_count` int DEFAULT '0' COMMENT '集数',
  `episodes_update_time` int DEFAULT '0' COMMENT '集数更新时间',
  `is_end` tinyint DEFAULT '0' COMMENT '是否完结0=未完结 1=已完结',
  `is_hot` tinyint DEFAULT '0' COMMENT '1=正在热播',
  `is_recommend` tinyint DEFAULT '0' COMMENT '1=推荐 0=否',
...
CREATE TABLE `video_episodes` (
  ...
  `title` varchar(255) DEFAULT NULL COMMENT '标题',
  `add_time` int DEFAULT '0' COMMENT '添加时间',
  `num` int DEFAULT '0' COMMENT '第几集',
  `video_id` int DEFAULT '0' COMMENT '所属时间',
  `play_url` text COMMENT '播放地址',
  `status` tinyint DEFAULT '0' COMMENT '0=不展示 1= 展示',
  `comment` int DEFAULT '0' COMMENT '评论数',
